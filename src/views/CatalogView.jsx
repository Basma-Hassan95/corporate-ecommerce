...
export default function CatalogView({ setActivePage, setSelectedProduct }) {
  const [selectedMaterial, setSelectedMaterial] = useState('All');
  const [selectedCategory, setSelectedCategory] = useState('All');
  const [searchQuery, setSearchQuery] = useState('');

  const materials = ['All', 'Leather', 'Wood'];
  const categories = ['All', 'Bifold Wallets', 'Slim Cardholders', 'Trifold Wallets', 'Wooden Box', 'Wooden Keychain', 'Desk Accessory'];
      ...
      <div className="container">
        ...
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <h1 style={{ fontSize: '3rem', fontFamily: 'var(--font-heading)', color: 'var(--text-dark-coffee)' }}>
            Full Bespoke Product Collection
          </h1>
          <p style={{ color: 'var(--text-dark-coffee)', opacity: 0.85, maxWidth: '600px', margin: '0.5rem auto 0' }}>
            Explore our complete range of personalized full-grain leather wallets and solid wood giftware.
          </p>
        </div>
        ...
        <div style={{ maxWidth: '540px', margin: '0 auto 2.5rem', position: 'relative' }}>
          <input
            ...
            placeholder="Search by wallet, box, keychain, or wood species..."
            style={{
              width: '100%',
                ...
                {materials.map((mat) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--radius-sm)',
                      cursor: 'pointer',
                      backgroundColor: selectedMaterial === mat ? 'var(--surface-linen)' : 'transparent',
                      color: selectedMaterial === mat ? 'var(--btn-coffee-bean)' : 'var(--text-dark-coffee)',
                      fontWeight: selectedMaterial === mat ? '700' : '400'
                    }}
                  >
                    {mat}
                  </button>
          ...
          <main>
            ...
            {filteredProducts.length === 0 ? (
              <div style={{ backgroundColor: 'var(--surface-white)', padding: '4rem 2rem', textAlign: 'center', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-light)' }}>
                <h3>No catalog items match your search or filter</h3>
                <p style={{ color: 'var(--accent-dusty-taupe)', margin: '0.5rem 0 1.5rem' }}>Try clearing your material selection or search term.</p>
                  ...
                  >
                    ...
                    <div style={{ padding: '1.2rem' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '0.6rem', borderTop: '1px solid var(--border-light)' }}>
                        ...
                        <span className="badge-taupe">Custom Laser</span>
                      </div>
                    </div>
                  </div>
                ))}
